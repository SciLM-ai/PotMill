def init_fit():
    ...
    with contextlib.suppress(Exception):
        import subdatapy.linalg  # noqa: F401  (warm the import cache if present)
    device_count = torch.cuda.device_count()
    if torch.cuda.is_available():
        torch.zeros(1, device="cuda")  # create the CUDA context now, not on first fit
    print(
        f"init_fit: torch {torch.__version__} | cuda_devices={device_count} | "
    ...
    if mlip == "ACE":
        ...
        for i, lst in enumerate(feature_names):
            if len(lst) == 1:
                ...
                if all(
                    lst[nu + 1 + k] <= nmaxes[nu - 2] and lst[2 * nu + k] <= lmaxes[nu - 2]
                    for k in range(nu - 1)
                ):
                    feature_indices.append(i)
    elif mlip == "SNAP":
...
def fit(
    ...
    if batch_ID is None:
        ...
        a_matr = torch.as_tensor(
            np.ascontiguousarray(a_map[:, feature_indices]), dtype=torch.float64, device=fit_device
        )
    else:
        parts = []
    ...
    for fold in range(n_fold):
        print(
            "===================== FOLD ", fold, " OF ", n_fold, "=====================", flush=True
        )
        if mlip == "ACE":
            print(
                "Hyperparameters rcut, nmax, lmax and eweight are "
                + rcuts_to_string(rcuts)
                + ", "
                + nmaxes_to_string(nmaxes)
                + ", "
                + lmaxes_to_string(lmaxes)
                + f" and {eweight:.3f}",
                flush=True,
            )
        else:
            print(
                "Hyperparameters rcut, 2Jmax and eweight are "
                + rcuts_to_string(rcuts)
                + ", "
                + twojmaxes_to_string(twojmaxes)
                + f" and {eweight:.3f}",
                flush=True,
            )
        ...
        fweights_train = 1.0 / torch.clamp(torch.abs(b_train[f_tr]), min=3.0)
        ...
        fweights_test = 1.0 / torch.clamp(torch.abs(b_test[f_te]), min=3.0)
        fweights_test = fweights_test / fweights_test.sum()

        a_stack = torch.cat(
            [
                eweights_train.unsqueeze(1) * a_train[e_tr],
                fweights_train.unsqueeze(1) * a_train[f_tr],
            ]
        )
        ...
        if not torch.all(torch.isfinite(beta_t)):
            print(
                f"WARNING: non-finite beta from '{fit_method}' solve (rank-deficient "
                f"design?) for hyperparameters {hyperparameters}",
                flush=True,
            )
...
class _FoldState:
    ...
    def __init__(self, p, device, dtype):
        ...
        def z():
            return torch.zeros((0, p + 1), device=device, dtype=dtype)
    ...
    def fold_batch(self, a, b, e_mask, part_rows, fold):
        ...
        for side, smask in (("tr", train), ("te", test)):
            for typ, tmask in (("E", e_mask), ("F", ~e_mask)):
                ...
                self.rmse[(side, typ, "0")] = self._merge(
                    self.rmse[(side, typ, "0")], self._aug(aa, bb)
                )
                self.rmse[(side, typ, "sq")] = self._merge(
                    self.rmse[(side, typ, "sq")], self._aug(aa, bb, torch.sqrt(w))
                )
                if side == "tr":
    ...
    def solve_and_rmse(self, eweight, rcond):
        ...
        alpha = eweight / self.Sw[("tr", "E")]
        beta = 1.0 / self.Sw[("tr", "F")]
        R_solve = torch.linalg.qr(
            torch.cat([alpha * self.solve["E"], beta * self.solve["F"]], dim=0), mode="r"
        ).R
        R = R_solve[: self.p, : self.p]
        d = R_solve[: self.p, self.p]
        ...
        if not bool(torch.all(torch.isfinite(x))):
            print(
                f"WARNING: non-finite beta from incremental solve (rank-deficient design?) eweight={eweight}",
                flush=True,
            )
        for s in self.SIDES:
            for t in self.TYPES:
