...
def featurize(
    ...
    feature_directory,
    only_cost=False,
    hyperparameters_noeweight=None,
    cost_nstructures=None,
    comm=None,
    rank=0,
    size=1,
):
    ...
    if config["FitSNAP"]["mlip"] == "ACE":
        if len(rcuts) == 1:
            fitsnap_config["ACE"]["rcutfac"] = rcuts_to_string(
                rcuts * (int(fitsnap_config["ACE"]["numTypes"]) ** 2)
            )
        else:
    ...
    if rank == 0:
        ...
        if not only_cost:
            ...
            if n_bad:
                print(
                    f"WARNING: featurize {os.getcwd()} a.npy has {n_bad} non-finite "
                    f"descriptor values (degenerate config?)",
                    flush=True,
                )
            np.save("a.npy", a_arr)
